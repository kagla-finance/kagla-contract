# @version 0.2.7
"""
@title "Zap" Depositer for Kagla BUSD pool
"""
...
def __init__(_pool: address, _token: address):
    ...
    self.pool = _pool
    self.token = _token
    _base_pool: address = KaglaMeta(_pool).base_pool()
    self.base_pool = _base_pool

    for i in range(N_COINS):
        coin: address = KaglaMeta(_pool).coins(convert(i, uint256))
        self.coins[i] = coin
    ...
    for i in range(BASE_N_COINS):
        coin: address = KaglaBase(_base_pool).coins(convert(i, uint256))
...
def add_liquidity(amounts: uint256[N_ALL_COINS], min_mint_amount: uint256) -> uint256:
    ...
    KaglaMeta(self.pool).add_liquidity(meta_amounts, min_mint_amount)
...
def remove_liquidity_imbalance(amounts: uint256[N_ALL_COINS], max_burn_amount: uint256) -> uint256:
    ...
    fee: uint256 = KaglaBase(_base_pool).fee() * BASE_N_COINS / (4 * (BASE_N_COINS - 1))
    ...
    KaglaMeta(_meta_pool).remove_liquidity_imbalance(amounts_meta, max_burn_amount)
    if withdraw_base:
        KaglaBase(_base_pool).remove_liquidity_imbalance(amounts_base, amounts_meta[MAX_COIN])
        leftover_amounts[MAX_COIN] = ERC20(_meta_coins[MAX_COIN]).balanceOf(self)
        if leftover_amounts[MAX_COIN] > 0:
            KaglaMeta(_meta_pool).add_liquidity(leftover_amounts, 0)
...
def calc_withdraw_one_coin(_token_amount: uint256, i: int128) -> uint256:
    ...
    if i < MAX_COIN:
        ...
        _base_tokens: uint256 = KaglaMeta(self.pool).calc_withdraw_one_coin(_token_amount, MAX_COIN)
        return KaglaBase(self.base_pool).calc_withdraw_one_coin(_base_tokens, i-MAX_COIN)
...
def calc_token_amount(amounts: uint256[N_ALL_COINS], is_deposit: bool) -> uint256:
    ...
    _base_tokens: uint256 = KaglaBase(self.base_pool).calc_token_amount(base_amounts, is_deposit)
    meta_amounts[MAX_COIN] = _base_tokens

    return KaglaMeta(self.pool).calc_token_amount(meta_amounts, is_deposit)
